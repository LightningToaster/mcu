// Valkor 2021-02-17
//TODO set should be split into set() and set_goal()?
...
#define TRANSITION_DELAY 7

//editing beyond here may require significant modification
...
public:
  ...
  void set_all(uint8_t r, uint8_t g, uint8_t b, bool instant = false, bool is_raw = false){
    for (uint8_t led = 0; led < NUM_PIXELS; led++){
      set(led,r,g,b,instant, is_raw);
    }
  }//set_all

  void set(uint8_t led, uint8_t r, uint8_t g, uint8_t b, bool instant = false, bool is_raw = false){
    if (instant){
      if (is_raw){
