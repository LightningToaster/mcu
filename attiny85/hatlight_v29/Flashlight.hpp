const uint8_t POWERS[] = {0,1,2,3,4 // (0-4) (v4, 2020-12-10)
  ,5,6,7,8,10 // (5-9)
...
class Flashlight{ 
  ...
  void change_brightness(int8_t index = -1){//TODO change this
    if (index == -1){
      switch (current_brightness_index){
        case DEFAULT_BRIGHTNESS_INDEX: current_brightness_index = 12; break;
        case 12: current_brightness_index = 24; break;
  ...
  void show_battery_level(long voltage){
    if (voltage < 3800){//warn user that battery is low
      blink(4,70);//count, delay
    }
  }
