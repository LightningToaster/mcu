...
public:
  ...
  uint8_t operate(bool wait_for_completion = false){//returns how many chips haven't reached their goal + 1
    ...
    if (status >= 1 and wait_for_completion){
      return operate(true);
    }
    return status;
  }//operate()

  void set_all(uint8_t r, uint8_t g, uint8_t b, bool is_raw = false){
    for (uint8_t led = 0; led < NUM_PIXELS; led++){
      set(led,r,g,b, is_raw);
    }
  }//set_all

  void set_all_goal(uint8_t r, uint8_t g, uint8_t b, bool is_raw = false){
    for (uint8_t led = 0; led < NUM_PIXELS; led++){
      set_goal(led,r,g,b, is_raw);
    }
  }//set_all

  void set(int8_t led, uint8_t r, uint8_t g, uint8_t b, bool is_raw = false){
    if (is_raw){ //convert to 0-31 if needed
      r = find_power_index(r);
      g = find_power_index(g);
...
//    }
    ...
    goals[led][1] = g;
    goals[led][2] = b;
    strip->setPixelColor(led, POWERS[r], POWERS[g], POWERS[b]);
  }//set

  void set_goal(int8_t led, uint8_t r, uint8_t g, uint8_t b, bool is_raw = false){
    if (is_raw){ //convert to 0-31 if needed
      r = find_power_index(r);
      g = find_power_index(g);
      b = find_power_index(b);
...
//  void setRandomGoal(uint8_t r, uint8_t g, uint8_t b, bool instant = false){
//    uint8_t led = random(NUM_PIXELS);
//    if (instant){
//      strip->setPixelColor(led, POWERS[r], POWERS[g], POWERS[b]);
//    }else{
//      goals[led][0] = r;
