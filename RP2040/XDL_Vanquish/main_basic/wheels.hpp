...
class Wheels { 
  ...
  static constexpr uint16_t THROTTLE_MAX   = 1420; // determines maximum fps allowed
  ...
  void set_speed(uint8_t power_setting) {
    uint32_t now = millis();
    if (now < ESC_ARM_DELAY || power_setting == 0) {
      speed_goal = THROTTLE_ARM;
      return;
    }
    if (power_setting > 9) { power_setting = 9; }
    int16_t new_speed = map(power_setting, 1, 9, THROTTLE_MIN, THROTTLE_MAX);
    speed_goal = static_cast<uint16_t>(new_speed);
